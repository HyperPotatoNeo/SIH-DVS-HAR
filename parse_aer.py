...
def cal_first_response(timestamps, theta, window=1000):
	...
	start_idx = 0
	print(o.shape)
	while (start_idx+1<o.shape[0] and o[start_idx+1]/o[start_idx] < theta):
		start_idx += 1
...
def clean_up_events(timestamps, xaddr, yaddr, pol, theta, window=100,
					key_idx=0):
	...
	"""
	if key_idx == 0:
		key_idx = cal_first_response(timestamps,theta, window)
...
def main(theta):
	...
	for filename in os.listdir(directory):
		if(filename.endswith('labels.csv') and filename!='gesture_mapping.csv'):
			...
			T,X,Y,polarity=clean_up_events(T,X,Y,polarity,theta)
			...
			for i in range(labels.shape[0]):
				...
				for k in range(X.shape[0]):
					...
					iterator+=1
					if(iterator%chunk_length==0 or k==X.shape[0]-1):
						frame*=255
						cv2.imwrite('parsed_data/train/'+str(action)+'/'+str(global_count)+'__'+str(theta)+'.png',frame)
						global_count+=1
