...
def loadaerdat(datafile='/tmp/aerout.dat', length=0, version=V3, debug=1, camera='DVS128'):
    ...
    while p < length:
        ...
        if(eventtype == EVT_DVS):  # this is a DVS event
            # print(addr)
            # print(xmask, ymask)
            # print(addr >> xshift, addr >> yshift)
            # x_addr = (addr >> xshift)&xmask
            # y_addr = (addr >> yshift)&ymask
            # print("D:", x_addr, y_addr)
...
directory='DVSGesturedataset/DvsGesture'
...
for filename in os.listdir(directory):
    if(filename.endswith('labels.csv') and filename!='gesture_mapping.csv'):
        ...
        for i in range(labels.shape[0]):
            ...
            end_time=labels[i,2]
            start_time=find_nearest(timestamps,start_time)
            end_time=find_nearest(timestamps,end_time)
            ...
            polarity=pol[start_index:end_index+1]
            X=np.asarray(X)
            if(X.shape[0]==0):
                continue
            #X=(X>>17)&0x00001FFF
