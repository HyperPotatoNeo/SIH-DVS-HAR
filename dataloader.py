...
class VideoGenerator:
	...
	def __init__(self, train_dir, test_dir, dims, batch_size=2, shuffle=True, file_ext=".npy"):
		...
		self.classname_by_id = {
		i: label for i, label in enumerate({os.path.basename(os.path.dirname(file)) for file in self.filenames_train})
		}

		self.id_by_classname = {label: i for i, label in self.classname_by_id.items()}
